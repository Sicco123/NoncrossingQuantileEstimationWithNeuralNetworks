...
class WindowGenerator():
  def __init__(self, input_width, label_width, shift,  #input_width = used history, label_width = number of outcomes beginning from the end of the window,
               train_df, val_df, test_df,              #Shift = the number of steps into the future
               label_columns=None):
    # Store the raw data.
  ...
  def plot(self, model=None, quantiles = [0.5], plot_col='gdp', max_subplots=3):
          ...
          if model is not None:
              ...
              for i in range(predictions.shape[-1]):
                    plt.scatter(self.label_indices, predictions[n,:, i],
                          marker='X', edgecolors=f'C{i}', label=f'Q-{quantiles[i]:.2f}',
                          c=f'C{i}', s=64, alpha = 0.5)
